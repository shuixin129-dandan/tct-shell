...
NAME=`grep "Name:" tct-shell.spec | awk '{print $2}'`
if [ -z "$NAME" ];then
...
VERSION=`grep "Version:" tct-shell.spec | awk '{print $2}'`
if [ -z "$VERSION" ];then
...
cp $PWD/tct-shell.spec $RPM_ROOT/SPECS
cd $RPM_ROOT/src_tmp
...
cd  $RPM_ROOT/SPECS
rpmbuild -ba tct-shell.spec --clean --define "_topdir $RPM_ROOT" --target=noarch
cd -
